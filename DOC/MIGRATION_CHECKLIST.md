...
- [ ] **الصفحات صفحةً بصفحة** (جارٍ):
  - [x] **رئيسية الباحث/الشركة** (`DashboardPage.vue`) — أُعيدت كتابتها Tailwind بالكامل (نُزع VCard/VRow/VCol/VList/VAlert/VProgressCircular/VTimeline/VChip/VBtn/VAvatar/VIcon). بديل جديد `BaseProgressRing` (SVG، لون دلالي)، وتوسعة `BaseButton` (polymorphic `to`→RouterLink + نغمات `tonal-brand/accent/emerald` + `align`). الأطفال المشتركون (StatCard/GamificationCard/OpportunityCard) يبقون Vuetify لحين دورهم. **مُتحقَّق حيًّا (نسختا باحث+شركة):** حلقة الثقة (قوس success بنسبة 77% + مركز «77»)، شبكة الإحصاء 4 أعمدة، العمود الرئيسي span-2، تنبيهات AI بحدّ ملوّن حسب النغمة، أزرار المساعد tonal-brand، أعمدة تحليلات الشركة (4) + قائمة المرشّحين (5). typecheck + 185 اختبار + build خضراء، 0 أخطاء console. **مكسب جانبي:** زالت تحذيرات «ref owner context» لأن VProgressCircular/VTimeline رُفعا.
